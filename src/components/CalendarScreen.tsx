...
export default function CalendarScreen({ session }: { session: any }) {
  ...
  const fetchEvents = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-4319e602/api/events`,
        {
          headers: {
  ...
  const handleRSVP = async (eventId: string) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-4319e602/api/events/${eventId}/rsvp`,
        {
          method: 'POST',
          headers: {
