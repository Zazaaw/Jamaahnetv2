...
export default function PostProductModal({ 
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-4319e602/api/upload-product-image`,
        {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-4319e602/api/marketplace/${type}`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${session.access_token}`,
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        toast.error(error.error || 'Gagal memposting produk');
      }
    } catch (error) {
      console.error('Error posting product:', error);
      toast.error('Gagal memposting produk');
    } finally {
      setLoading(false);
    }
  };
