...
export default function MarketplaceScreen({ 
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-4319e602/api/marketplace/${activeTab}`,
        {
          headers: {
  ...
  const fetchProductRating = async (productId: string) => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-4319e602/api/products/${productId}/reviews`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
          },
      ...
      if (reviews.length > 0) {
        ...
        setProductRatings(prev => ({
          ...prev,
          [productId]: { avg: avgRating, count: reviews.length }
        }));
      }
    } catch (error) {
